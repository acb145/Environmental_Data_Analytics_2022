...
#Create a map
HUC8.Neuse.Plot <- ggplot() +
                    geom_sf(data = huc8_sf, color = "orange") +
                    geom_sf(data = neuse, color = "purple")
HUC8.Neuse.Plot
...
HUC12.Wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>% 
  mutate(HUC_8 = substr(HUC_12, 0, 8)) %>% 
  group_by(HUC_8) %>% 
  summarise(TotalAvgWindEnergy = sum(AvgWindEnergy))

#Join to HUC_8 features
Wind.Join <-  merge(x = huc8_sf,
                           y = HUC12.Wind, 
                           by.x = "HUC_8", 
                           by.y = "HUC_8")

#View the outputs
mapview(Wind.Join, zcol = "TotalAvgWindEnergy")
...
triangle.sf.utm <- counties_sf_utm %>% 
                      filter(NAME %in% c("Chatham", "Durham", "Orange", "Wake"))

mapview(triangle.sf.utm)
  
#Grab the intersecting HUC_8s
selected.HUCs <- huc8_sf_utm [triangle.sf.utm,]

mapview(selected.HUCs)

#Intersect the HUC_8s
clipped.HUCs <- huc8_sf_utm %>% 
                  st_intersection(triangle.sf.utm)

mapview(clipped.HUCs)
