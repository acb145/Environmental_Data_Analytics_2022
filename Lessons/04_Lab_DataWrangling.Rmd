...
# you can use an integer whereas when filtering by lake name you have to use quotes because
# it is a factor


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TuesLake.1990s <- NTL.phys.data %>% 
...
> Answer: Easier to just use range of values you are looking for rather than finding row numbers
...
NTL.phys.data.TuesLake.1990s.July <- NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1999)) %>% 
...
NTL.phys.data.Ex4 <- NTL.phys.data.TuesLake.1990s.July %>% 
  filter(depth == 0) %>% 
...
NTL.phys.data.irradiance.gather <- NTL.phys.data %>% 
  pivot_longer(irradianceWater:irradianceDeck, names_to = "location", values_to = "irradiance")


#Exercise 6: spread temperatureC into more than one column based on the depth

NTL.phys.data.tempC.spread <- NTL.phys.data %>% 
