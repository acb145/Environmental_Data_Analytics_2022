...
> Answer: Null: Ozone levels are not below the threshold for good.
          Alternative: Ozone levels are below the threshold for good.
...
> There is  a large positive skew on the dataset with some outlier values at higher AQI values. The majority of the data seems to fall below the threshold for good air quality with some outliers with much higher values.
...
Plot2.1 <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(aes(fill = as.factor(Year)), alpha = 0.5) +
  labs(x = "Ozone AQI Level", y = "Density", fill = "") +
  scale_fill_viridis(option = "C", discrete = TRUE)
Plot2.1

# Box Plot

Plot2.2 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, label = c("b", "a")) + #marking text at max of y variable
  labs(x = "", y = "Ozone AQI Level") +
  scale_color_viridis(option = "C", discrete = TRUE)
Plot2.2

# Violin Plot

Plot2.3 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_violin(aes(fill = as.factor(Year)), draw_quantiles = 0.5) +
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, label = c("b", "a")) +
  labs(x = "", y = "Ozone AQI Level", fill = "")
Plot2.3
...
EPAair.cor <- cor(EPAair.proc)
corrplot(EPAair.cor, method = "ellipse")
corrplot.mixed(EPAair.cor, upper = "ellipse")
...
EPAair.AIC <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanLng + meanLat)
step(EPAair.AIC)
...
EPAair.Model <- lm(data = EPAair, PM2.5 ~ Ozone + Month + meanLng + meanLat)
summary(EPAair.Model)
