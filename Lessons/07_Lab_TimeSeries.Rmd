...
plot(Temp_BR_processed)
plot(Rainfall_BR_processed)
...
RainfallTrendPlot <- ggplot(Rainfall_BR_processed, aes(Date, Rainfall)) +
                geom_line() +
                geom_smooth(method = "lm") 
RainfallTrendPlot

TempTrendPlot <- ggplot(Temp_BR_processed, aes(Date, Temperature_C)) +
                geom_line() +
                geom_smooth(method = "lm") 
TempTrendPlot
...
Rainfall_BR_ts <- ts(Rainfall_BR_processed$Rainfall, 
                     start = c(1901, 1), frequency = 12)

Temp_BR_ts <- ts(Temp_BR_processed$Temperature_C, 
                     start = c(1901, 1), frequency = 12)
...
RainfallDecomp <- stl(Rainfall_BR_ts, s.window = "periodic")
plot(RainfallDecomp)

TempDecomp <- stl(Temp_BR_ts, s.window = "periodic")
plot(TempDecomp)
...
Rainfall.Components <-as.data.frame(RainfallDecomp$time.series[,1:3])
Rainfall.Components <- mutate(Rainfall.Components,
        Observed = Rainfall_BR_processed$Rainfall,     
        Date = Rainfall_BR_processed$Date)

Temp.Components <-as.data.frame(TempDecomp$time.series[,1:3])
...
RainfallTrendOverlay <- ggplot(Rainfall.Components, aes(Date, Observed)) +
  geom_line()+
  geom_line(aes(Date, trend))
RainfallTrendOverlay

TempTrendOverlay <- ggplot(Temp.Components, aes(Date, Observed)) +
  geom_line()+
  geom_line(aes(Date, trend))
TempTrendOverlay
...
Rainfall.SMK<- SeasonalMannKendall(Rainfall_BR_ts)
summary(Rainfall.SMK)

Temp.SMK<- SeasonalMannKendall(Temp_BR_ts)
summary(Temp.SMK)
